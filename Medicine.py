...
df_medicine = Med_Data("/Users/aditirajesh/Desktop/program_files/python/python_2sem/CMS_project/med.csv")
df_bandages = Med_Data("/Users/aditirajesh/Desktop/program_files/python/python_2sem/CMS_project/bandages.csv")
...
def sidebar():
    ...
    with st.sidebar:
        ...
        def prescription(df):
            ...
            value = st.number_input("Enter Amount",step=1,min_value=0)
            #counter = 0 
            ...
            if st.button("Prescribe"):
                        ...
                        if i[6] < value:
                            st.error("Not enough in stock.")
                                ...
                                if i in df.expired()[1]:
                                    ...
                                    update_table = df.update(i[0],"Stock",i[6]-value)

                                    if update_table == "Invalid ID":
                                        st.error("No such name found in table") 
                ...
                if counter == 0:
                    st.error("No such name found in the table")
